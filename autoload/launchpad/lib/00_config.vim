...
func launchpad#lib#00_config#build()
	...
	call launchpad#job(launchpad#lib#00_config#sanitize_cmd(s:json.build[0].cmd), #{env: get(s:json.build[0], "env", {}), out_cb: function('launchpad#out_cb'), exit_cb: function('launchpad#build_cb')})
endfunc

func launchpad#lib#00_config#launch()
	call launchpad#job(launchpad#lib#00_config#sanitize_cmd(s:json.launch[0].cmd), #{env: get(s:json.launch[0], "env", {}), out_cb: function('launchpad#out_cb'), err_cb: function('launchpad#out_cb'), exit_cb: function('launchpad#launch_cb')})
endfunc

func launchpad#lib#00_config#parse_output(l)
	if s:json.build[0]->has_key("kind")
		let Func = function(printf("launchpad#lib#parse_output_%s", s:json.build[0].kind), [a:l])
		return Func()
	endif
	return 0
endfunc
